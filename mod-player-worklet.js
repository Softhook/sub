...
class Channel {
    ...
    nextOutput() {
        ...
        try {
            // Make sure the instrument has valid sample data
            if (!this.instrument.bytes || !this.instrument.bytes.length) {
                return 0.0;
            }
            
            // Calculate array index - floor the index for safety
            const sampleIndex = Math.floor(this.sampleIndex);
            
            // Make sure the index is within bounds
            if (sampleIndex < 0 || sampleIndex >= this.instrument.bytes.length) {
                return 0.0;
            }
            
            // Get the sample value
            const sample = this.instrument.bytes[sampleIndex];
            
            // Update sample index for next frame
            this.sampleIndex += this.sampleSpeed;
            
            // Handle looping samples
            ...
            // Normalize the sample value and apply volume
            // Increased output level by 30% for better audibility
            return (sample / 128.0) * (this.currentVolume / 64) * 1.3;
        } catch (err) {
    ...
    play(note) {
        ...
        if (note && note.instrument) {
            // Safely access the instrument
            const instruments = this.worklet.mod?.instruments;
            if (instruments && Array.isArray(instruments) && instruments.length > note.instrument) {
                this.setInstrument = instruments[note.instrument];
                // Only set volume if the instrument exists and has a volume property
                if (this.setInstrument && typeof this.setInstrument.volume !== 'undefined') {
        ...
        if (note && note.period) {
            ...
            // Safely access finetune value
            const finetune = (instrument && typeof instrument.finetune !== 'undefined') ? instrument.finetune : 0;
            
            // Ensure we have a valid period value
            if (typeof note.period === 'number' && !isNaN(note.period)) {
    ...
    effect(noteEffect) {
        ...
        // Safety check for noteEffect parameter
        if (!noteEffect) return;
        
        const { hasEffect, effectId, effectData, effectHigh, effectLow } = noteEffect;
...
class ModPlayerWorklet extends AudioWorkletProcessor {
    ...
    nextRow() {
        try {
            // Safety check for mod data
            if (!this.mod || !this.mod.patternTable || !this.mod.patterns) {
                console.error("Invalid mod data in nextRow");
                return;
            }
            ...
            else if (this.rowIndex >= 64) { // Changed from == to >= for safety
                this.rowIndex = 0;
                ++this.position;
            }
            ...
            // Ensure patternTable access is safe
            if (this.position < 0 || this.position >= this.mod.patternTable.length) {
                console.error("Invalid pattern position:", this.position);
                this.position = 0;
            }

            // Get pattern data with safety checks
            const patternIndex = this.mod.patternTable[this.position];
            if (patternIndex === undefined || patternIndex < 0 || patternIndex >= this.mod.patterns.length) {
                console.error("Invalid pattern index:", patternIndex);
                return;
            }
            
            const pattern = this.mod.patterns[patternIndex];
            if (!pattern || !pattern.rows) {
                console.error("Invalid pattern or missing rows");
                return;
            }
            
            // Ensure row index is valid
            if (this.rowIndex < 0 || this.rowIndex >= pattern.rows.length) {
            ...
            if (!row || !row.notes) {
                console.error("Invalid row or missing notes at position", this.position, "row", this.rowIndex);
                return;
            }

            // Process each channel
            const channelCount = Math.min(4, row.notes.length, this.channels.length);
            ...
            console.error("Error in nextRow:", err);
        }
    }
    ...
    nextOutput() {
        ...
        try {
            ...
            if (this.outputsUntilNextTick <= 0) {
                ...
                if (isNaN(this.outputsPerTick) || this.outputsPerTick <= 0) {
                    console.error("Invalid outputs per tick:", this.outputsPerTick);
                    this.outputsPerTick = this.sampleRate * 60 / 125 / 4 / 6; // Default 125 BPM
                }
            ...
            // Calculate output from all channels
            let rawOutput = 0;
            for (let i = 0; i < this.channels.length; i++) {
                const channelOutput = this.channels[i].nextOutput();
                if (!isNaN(channelOutput)) {
                    rawOutput += channelOutput;
                }
            }
            
            // Apply soft clipping and amplify the output slightly for better audibility
            return Math.tanh(rawOutput * 1.5);
        } catch (err) {
